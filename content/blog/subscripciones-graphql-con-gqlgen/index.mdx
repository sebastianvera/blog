...
utilizar [gqlgen] en el backend, apollo en el frontend y desarrollar una
simple aplicación.
...
al mapa que contiene los canales, esto es necesario dado que los mapas en go [no
permiten lectura durante una
escritura](https://stackoverflow.com/questions/36167200/how-safe-are-golang-maps-for-concurrent-read-write-operations).
...
¿Por qué no podemos ocupar el servidor con una configuración por defecto
([handler.NewDefaultServer](https://github.com/99designs/gqlgen/blob/master/graphql/handler/server.go#L31))?
Basicamente necesitamos modificar la función `CheckOrigin` o no podremos acceder
por medio websockets a nuestra API desde un origen distinto al de nuestro
servidor.
...
- [Codigo del servidor en Github](https://github.com/sebastianvera/ghreviews)
